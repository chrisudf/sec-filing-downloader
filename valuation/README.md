...
- **近零利润守卫**：情景 eps1<=0 或 opm<2% 时 PE 腿 n.m. 退出综合（blend_methods 记录），
  红旗区给 P/S 参考价（facts.ps_band，不入综合）；
- **DCF 现金流锚守卫**（0023）：TTM FCF <= 2% 营收（= 『DCF权益/TTM FCF』护栏跑不起来
  的同一条谓词）、或 dcf_ps<=0、或显式期现值非正时，DCF 腿 n.m. 退出综合。校验一条腿
  的护栏无法生效时，这条腿不该有投票权。dcf_ps 仍照算、照进 valuation.json 与 DCF 页；
- **腿间倍数互校**（0023）：PE 腿目标权益换算回 EV/EBIT，与 SOTP 的 `seg1_share*m1+
  (1-seg1_share)*m2` 比，偏离 >±25% 打 🟡（AMZN 实测 +53%/+36%/+21%，META +5%/+8%/+9%）；
- **综合区间**（0023）：`scenarios[*].blend_range` = 参与综合的腿的 [min,max]，
  离散度黄旗与 Excel A26 都带上它——点估计不该被当成精度读；
- **双口径分位**（0026）：`trading_range.trailing_basis` 把 `trailing_nolag` 提成
  一等读数——NTM 口径结构性滞后约一年（四标的实测 305/325/415/423 天），trailing
  口径没有这个滞后且同样过滤。两者**并列不合并**：分母不同（未来12个月 vs 过去
  12个月），比值即 EPS 增速因子，实测极差 1.5~3.8x，单标量折算不成立，因此不提供
  任何折算或相减字段。trailing 侧自身的滞后也照报（带一次性重估的票会被同一笔
  损益把两个口径一起堵住）；
